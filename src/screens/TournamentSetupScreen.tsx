  ...
  return (
    <form onSubmit={handleSubmit} className="grid w-full grid-cols-1 gap-4 lg:grid-cols-[1fr_1fr_2fr] lg:gap-6">
      <RosterSelection
        ...
        playerInstructions="Drag people between lists or reorder the player order."
        emptyPlayersMessage="Click a user to add them here. Need 2+."
      />
        ...
        {format === 'round_robin' && (
          <fieldset className="m-0 border-none p-0">
            ...
            <div className="flex gap-1 rounded-(--radius-md) bg-sunken p-1">
              {([[1, 'Once'], [2, 'Twice']] as const).map(([count, label]) => (
                <button
                  key={count}
                  type="button"
                  ...
                  className={
                    ...
                    (matchesPerPair === count ? 'bg-card text-ink shadow-sm' : 'bg-transparent text-ink-muted hover:text-ink')
                  }
                >
                  {label}
                </button>
